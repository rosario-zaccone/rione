...
\newcommand{\reportversion}{v0.1.27}
\newcommand{\reportlastmodified}{2026-07-27}
...
Metrics and health checks say whether a service is up and how it is performing, but diagnosing why a specific request failed usually requires reading the actual log lines a service produced at that moment. Before this addition, logs were only available per container through \texttt{docker compose logs <service>}, with no way to search or correlate log lines across the five backend services at once, which is impractical once a failure spans more than one service (for example, a request that touches the API Gateway, the Social Service, and the User Service in sequence).

The project addresses this with Grafana Loki as the log store and Promtail as the collector. Promtail discovers running containers directly through the Docker socket (\texttt{docker\_sd\_configs}), so no code or logging configuration changes were required in any service: every container's stdout/stderr, already captured by the Docker \texttt{json-file} log driver, is picked up automatically. Promtail labels each log stream with the originating container and Docker Compose service name before shipping it to Loki, which keeps logs queryable per service the same way Prometheus metrics are queryable per \texttt{job}. Loki is registered as a second Grafana datasource alongside Prometheus, so logs can be explored and time-correlated with the existing metrics dashboards from the same Grafana instance, without introducing a separate tool such as the ELK stack, which would add a heavier operational footprint than this local, Docker-Compose-based deployment needs.
