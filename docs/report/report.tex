...
\newcommand{\reportversion}{v0.1.19}
\newcommand{\reportlastmodified}{2026-07-09}
...
\begin{itemize}
    \item \textbf{Java 21}: main programming language for backend services.
    \item \textbf{Spring Boot 4.1.0}: base framework for the backend microservices.
    \item \textbf{Spring Web MVC}: implementation of REST APIs exposed by the services.
    \item \textbf{Spring Data JPA}: persistence layer for relational data.
    \item \textbf{PostgreSQL}: relational database used by the services that require persistence.
    \item \textbf{Apache Kafka}: event broker used for asynchronous communication between social, post, and notification workflows.
    \item \textbf{Spring Security}: authentication and authorization through bearer tokens and service-to-service security rules.
    \item \textbf{Springdoc OpenAPI}: generation and publication of API documentation.
    \item \textbf{React and Vite}: frontend implementation and local development environment.
    \item \textbf{Docker and Docker Compose}: containerization and local orchestration of services, databases, Kafka, Prometheus, Grafana, and pgAdmin.
    \item \textbf{Prometheus and Grafana}: collection and visualization of technical and business metrics.
    \item \textbf{Maven}: build, dependency management, and test execution.
    \item \textbf{GitHub and GitHub Actions}: version control and continuous integration.
\end{itemize}
...
The test suite is executed with Maven. During continuous integration, GitHub Actions runs \texttt{verify} on each service and then on the full Maven reactor. This means that a change is validated both in its own module and in the context of the whole project.
...
The project uses Domain-Driven Design as required by the exam assignment. DDD was adopted to keep the software model close to the business domain of a neighbourhood-based social network, where concepts such as users, neighbourhoods, posts, social relationships, and notifications have precise meanings. This approach also helped separate business decisions from technical details such as REST controllers, persistence, messaging, and infrastructure configuration.

\subsection{Strategic Design}

Strategic design was used to understand the problem space before defining the implementation structure. The domain was divided into clear business areas, each with its own model, responsibilities, and language. This was useful for identifying service boundaries and for avoiding a single shared model that would mix unrelated concerns.
...
These contexts also correspond to the main business microservices of the system. This mapping keeps each service focused on a coherent set of rules and reduces coupling between services.
...
For each bounded context, a UML domain model was designed and is presented later in the report. These diagrams identify the main aggregates, entities, value objects, and relationships used by the services. They provide a bridge between the requirements documentation and the implementation, making the domain structure easier to inspect before looking at the source code.
...
The business microservices follow a clean and hexagonal organization. Their code is divided into domain, application, and infrastructure layers. The domain layer contains the core business model and rules; the application layer coordinates use cases and defines ports; the infrastructure layer contains adapters for REST APIs, persistence, messaging, configuration, and external integrations.

This structure keeps dependencies directed toward the inner layers. Controllers, repositories, Kafka consumers, REST proxy clients, and other technical details depend on the application and domain model, while the domain model does not depend on Spring, databases, HTTP, or Kafka. This makes the business logic easier to test and reduces the impact of changing an external technology. Figure~\ref{fig:clean-architecture} summarizes this dependency direction.
...
An event-driven approach is used where asynchronous communication is more appropriate than direct synchronous calls. In particular, the Notification Service reacts to events produced by other services instead of being called directly by each workflow that may generate a notification.

Kafka acts as the event broker. The Post Service and Social Service publish events on Kafka topics when relevant domain actions occur, and the Notification Service consumes those events to create or deliver notifications. This design reduces coupling between the services that produce business events and the service that handles notifications. It also makes the notification workflow more scalable, because event producers do not need to wait for notification processing to complete before finishing their own use cases.

\subsection{Application Security}

Security is managed with Spring Security and JWT-based stateless authentication. The User Service is responsible for user registration and login: credentials are validated there, passwords are stored as hashes rather than plain text, and a JWT is returned after a successful login. Clients then send the token in the \texttt{Authorization: Bearer <token>} header when calling protected APIs. Each backend service validates the token before executing protected operations, so authentication does not depend on server-side HTTP sessions and can work naturally in a distributed microservices deployment.
...
The API Gateway is the public entry point of the backend. It exposes the routes consumed by the frontend and forwards requests to the internal services, keeping clients independent from the internal service topology. It does not own a domain model; its responsibility is API composition, routing, and boundary-level access to the backend capabilities. Its public API surface is illustrated in Figure~\ref{fig:api-gateway-openapi}.
...
The Social Service manages relationships between users inside the local social network. Its domain model focuses on neighbour requests, neighborships, and blocks. It references users by identifier and uses proxy clients when it needs additional user data from the User Service. Its API contract is shown in Figure~\ref{fig:social-service-openapi}, its domain model in Figure~\ref{fig:social-service-domain}, and its component-and-connector view in Figure~\ref{fig:social-service-cc}.
...
In addition to the core microservice, DDD, and hexagonal architecture choices, the project includes operational patterns that make the system easier to run and observe. The most relevant ones are health checks and application metrics. They do not implement business behavior directly, but they are important because a distributed system must expose enough runtime information to understand whether services are alive, reachable, and behaving as expected.

\subsection{Health API}

Each backend service exposes health information through Spring Actuator. The health endpoint is made available without user authentication so that Docker Compose, monitoring tools, or deployment infrastructure can check whether a service is running and ready to receive traffic. This is especially useful in a microservices architecture, where a failure may affect only one service while the rest of the system continues to run.

The health API is intentionally separated from the business OpenAPI contracts. Business APIs describe domain capabilities such as users, posts, social relationships, and notifications, while Actuator endpoints describe operational state. In the services, health and Prometheus endpoints are explicitly allowed by the security configuration, avoiding the need for monitoring tools to use a normal user token.

\subsection{Circuit Breaker}

The project also uses the Circuit Breaker pattern to limit cascading failures during synchronous service calls. The API Gateway applies circuit breakers to the routes that forward traffic to the User, Social, Post, and Notification services. When a downstream service becomes slow or repeatedly fails, the circuit can open and stop sending new calls for a short period, giving the failing service time to recover and preventing request queues from growing across the system.
...
Application metrics are collected with Micrometer and exposed in Prometheus format through Spring Actuator. The services expose \texttt{/actuator/prometheus}, and each service adds the \texttt{application} tag to its metrics so that Prometheus and Grafana can distinguish data coming from the API Gateway, User Service, Social Service, Post Service, and Notification Service.

The project includes both technical and business-oriented metrics. Technical metrics are provided by the Spring Boot and JVM instrumentation and help observe runtime behavior such as HTTP requests, memory usage, and service availability. Business metrics are added where they provide useful insight into the application domain. For example, the User Service exports gauges for registered users, new users created today, and active users in the last five minutes. The Post Service exports gauges derived from the event store, such as total posts, posts created today, comments created today, and reactions created today.
